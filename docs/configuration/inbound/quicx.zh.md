...
保底冗余率（百分比，默认 `0`，即干净链路上一个校验包都不发）。

纯反应式 FEC 要等对端上报丢包后才发第一行，高 RTT 或低速率链路上，第一个突发可能在反馈到达前
就滚出窗口了。设为 `2`～`5` 时，发送端在干净链路上也保持这个比例的校验流量，让首个丢包立刻
有修复行可用；代价是干净链路不再零冗余。该值仍然受 `max_overhead_percent` 上限约束，建议只在
高 RTT / 低速率链路上先小值实测。
